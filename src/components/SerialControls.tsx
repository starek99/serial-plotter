...
export function SerialControls() {
  const { isPaused, setIsPaused, data, isConnected, connect, disconnect } =
    useSerialContext();
  ...
  useState(() => {
    const validChannels = channels.filter(
      (ch) => typeof ch === "string" && !ch.includes("�") && ch.trim() !== ""
    );
              ...
              <div className="space-y-2">
                {channels.map((channel) => (
                  ...
                  >
                    ...
                    />
                    <div
                      className={`h-3 w-3 rounded-full ${getLineColor(
                        channel
                      )}`}
                    />
                    <Label
                ...
                {latestReadings.map((reading, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className={`h-2 w-2 rounded-full ${getLineColor(
                          reading.channel
                        )}`}
                      />
